...
class MongoSpoutTask implements Callable<Boolean>, Runnable, Serializable {
    ...
    private List<ServerAddress> serverAddresses = null;
    // Keeps the running state
    private AtomicBoolean running = new AtomicBoolean(true);
    private String[] collectionNames;
    private DBObject query;

    public MongoSpoutTask(LinkedBlockingQueue<DBObject> queue, List serverAddresses, String dbName, String[] collectionNames, DBObject query) {
        this.queue = queue;
        ...
        initializeMongo(serverAddresses, dbName);
    }

    private void initializeMongo(List serverAddresses, String dbName) {
        // Open the db connection


        // Create mongo instance
        mongo = new Mongo(serverAddresses);
        //get read from slaves as preferred method
    ...
    public Boolean call() throws Exception {
        ...
        // provide the query object for timestamp
        ...
        LOG.info("Open tailable MongoDB cursor");
        ...
        while (running.get()) {
            try {
                ...
                if (this.cursor.hasNext()) {
                    ...
                    // push to the queue only record that has collections we are interested in
                    DBObject object = this.cursor.next();
                    String ns = object.get("ns").toString();
                    if (ns.startsWith("cp."))  {
